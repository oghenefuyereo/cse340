...
requiredMethods.forEach(({ method, controller }) => {
  if (!controller[method]) {
    ...
    console.log(`Method ${method} is defined in invController.`);
  }
});
...
router.get(
  ...
  utilities.handleErrors(async (req, res) => {
    try {
      ...
      // Render the classification view and pass inventory items
      res.render("inventory/classification", {
        title: "Inventory Items",
        grid: utilities.buildClassificationGrid(inventoryItems), // Pass grid HTML to the view
      });
    } catch (error) {
